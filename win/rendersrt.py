...
def main():
    ...
    if args.media_filepath and args.subtitle_file_path and args.language and args.output_file_path:
        ...
        if media_filepaths and valid_subtitle_paths and output_path:
            ...
            ffmpeg_language_code = args.language
            ...
            if subtitle_streams_data and subtitle_streams_data != []:
                ...
                src_subtitle_stream_timed_subtitles = subtitle_stream_parser.timed_subtitles_of_language(ffmpeg_language_code)

                if ffmpeg_language_code in subtitle_stream_parser.languages():
                    print("Is '%s' subtitle stream exist       : Yes" %(ffmpeg_language_code.center(3)))
                else:
                    print("Is '%s' subtitle stream exist       : No" %(ffmpeg_language_code.center(3)))

                    #result = render_subtitle_into_media(media_filepaths[0], media_type, valid_subtitle_paths[0], ffmpeg_language_code, output_path, error_messages_callback=None)

                    widgets = [f"Rendering '{ffmpeg_language_code}' subtitles with {media_type} : ", Percentage(), ' ', Bar(marker="#"), ' ', ETA()]
                    pbar = ProgressBar(widgets=widgets, maxval=100).start()
                    subtitle_renderer = MediaSubtitleRenderer(subtitle_path=valid_subtitle_paths[0], language=ffmpeg_language_code, output_path=output_path, progress_callback=show_progress, error_messages_callback=show_error_messages)
                    result = subtitle_renderer(media_filepaths[0])
                    pbar.finish()

        if result and os.path.isfile(result):
            print("Subtitles rendered {} saved as    : {}".format(media_type, output_path))
